...
trait IERC721<TContractState> {
    ...
    // Số lượng NFT address sở hữu
    fn balance_of(self: @TContractState, account: ContractAddress) -> u256;
    // Truy vấn xem 1 address có phải là authorized operator cho 1 address khác hay không
    fn is_approved_for_all(
        self: @TContractState, owner: ContractAddress, operator: ContractAddress
    ) -> bool;
    // Trả về địa chỉ sở hữu NFT
    fn owner_of(self: @TContractState, token_id: u256) -> ContractAddress;
    // Trả về địa chỉ được approved cho `_tokenId` NFT
    fn get_approved(self: @TContractState, token_id: u256) -> ContractAddress;
    // Enable hoặc disable cho bên thứ 3 ("operator") có thể quản lý tất cả tài sản của msg.sender
    fn set_approval_for_all(
        ref self: TContractState, operator: ContractAddress, approved: bool
    );
    // Thay đổi hoặc xác nhận lại địa chỉ đã được approved cho NFT
    fn approve(ref self: TContractState, to: ContractAddress, token_id: u256);
    // Transfer ownership của NFT, nhưng msg.caller phải confirm rằng địa chỉ `_to` có khả năng để
    // nhận NFT hoặc không sẽ mất vĩnh viễn
    fn transfer_from(
